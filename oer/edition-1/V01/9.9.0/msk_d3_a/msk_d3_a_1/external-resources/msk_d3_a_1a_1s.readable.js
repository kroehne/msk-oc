...
// 	}
	...
	triggerInputValidationEvent () {
		if ( this.fsm.triggerEvent ) {
			this.fsm.triggerEvent( 'ev_InputValidation_' + 'msk_d3_a_1a_1s'.replace("msk_","") );
...
/* harmony export */ });
/* harmony import */ var _textareaInserts__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./textareaInserts */ "./libs/textareaInserts.js");


class inputInserts extends _textareaInserts__WEBPACK_IMPORTED_MODULE_0__.textareaInserts {
	...
	isOpRE ( Ops, mult, res, resOpt ) {
		...
		if ( res !== null && res !== undefined ) {
			...
			re = new RegExp( `^(?:(?:${res}\\s*(=|\u003d)\\s*)${mrs}|${mrs}(?:\\s*(=|\u003d)\\s*${res})${ resOpt ? '?' : ''})$` );
		} else {
			re = new RegExp( res === null ? `^${mrs}$` : `(?:^|\\s|[^0-9,.])${mrs}(?:\\s|[^0-9,.]|$)` );
		}
		return this.div.innerHTML.trim().match(re);
	}
...
/* harmony import */ var _libs_inputInserts__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./libs/inputInserts */ "./libs/inputInserts.js");
...
const ii = new _libs_inputInserts__WEBPACK_IMPORTED_MODULE_2__.inputInserts( '#container', {
...
//////////////////////////////////////
	scoreDef: function () {
		if ( this.isDiffRE( [ [1.9, 0.6] ], 1.3, false ) )		return 1;

		if ( !this.div.innerHTML.trim().length ) return -1;

		if ( this.isDiffRE( [ [ 1.9, 1.3 ] ], 0.6, false ) ||
			this.isDiffRE( [ [ 1.3, 0.6 ] ], 0.7, false ) ||
			this.isDiffRE( [ [ 1.3, 1.9 ] ], 0.6, false ) ||
			this.isDiffRE( [ [ 0.6, 1.9 ] ], 1.3, false ) )		return -3;

		if ( this.isSumRE( [ [ 1.3, 0.6 ], [ 0.6, 1.3 ] ], 1.9, false ) )	return -4;

		if ( this.isSumRE( [ [ 1.3, 1.9 ], [ 1.9, 1.3 ] ], 0.6, false ) ||
			this.isSumRE( [ [ 1.3, 1.9 ], [ 1.9, 1.3 ] ], 3.2, false ) )		return -5;
...
/******/ })()
;
//# sourceMappingURL=msk_d3_a_1a_1s.readable.js.map