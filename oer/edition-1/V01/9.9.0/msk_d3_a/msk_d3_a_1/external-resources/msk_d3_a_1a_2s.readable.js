...
// 	}
	...
	triggerInputValidationEvent () {
		if ( this.fsm.triggerEvent ) {
			this.fsm.triggerEvent( 'ev_InputValidation_' + 'msk_d3_a_1a_2s'.replace("msk_","") );
...
//////////////////////////////////////
	scoreDef: function () {
		if ( this.isDiffRE( [ [ 4.08, 0.08, 0.5 ] ], 3.5, false ) ||
			this.isDiffRE( [ [ 4.08, 0.5, 0.08 ] ], 3.5, false ) )		return 1;

		if ( !this.div.innerHTML.trim().length ) return -1;

		if ( this.isDiffRE( [ [ 4.08, 0.58 ] ], 3.5, true ) )		return -3;

		if ( this.isDiffRE( [ [ 4.08, 4 ] ], 0.08, true ) ||
			this.isDiffRE( [ [ 4, 3.5 ] ], 0.5, true ) )		return -4;

		if ( this.isDiffRE( this.allCombPerm( [ 4.08, 4, 3.5 ] ) ) )		return -5;

		if ( this.div.innerHTML.trim().match( /[0-9,.]+\s*(?:\+)\s*[0-9,.]+/ ) )	return -6;
...
/******/ })()
;
//# sourceMappingURL=msk_d3_a_1a_2s.readable.js.map