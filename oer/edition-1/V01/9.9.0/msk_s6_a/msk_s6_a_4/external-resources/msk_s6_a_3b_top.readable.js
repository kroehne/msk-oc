...
// 	}
	...
	triggerInputValidationEvent () {
		if ( this.fsm.triggerEvent ) {
			this.fsm.triggerEvent( 'ev_InputValidation_' + 'msk_s6_a_3b_top'.replace("msk_","") );
...
// jetzt Kopie von msk_s6_b_2d.js
...
const barX = iconWidthAll+25+labWidth;
const barHeight = 40;
const barWidth = width-barX-labWidth-20 -iconWidthAll;
...
const io = new _libs_barSlider__WEBPACK_IMPORTED_MODULE_2__.barSlider_freePaint_freeLabels_insertButtons( base, {
	...
	insertIconDefs: [{
		x: width-iconWidthAll-5,
		y: (height-2*(iconWidthAll+2))/2,
		width: iconWidth, height: iconWidth,
		framePadding: iconPadding,
		texts: ['%','€'],
	}],
	...
	scoreDef: function () {
		if ( !this.freeLabels[5].textObj )		return -2; 	// not initialized yet

		if ( (0,_libs_common__WEBPACK_IMPORTED_MODULE_3__.isBetween)( this.pos, 67/100, 83/100 ) &&
			this.freeLabels[0].textObj.value.trim().match( /^0+ ?%$/ ) &&
			this.freeLabels[2].textObj.value.trim().match( /^0*75 ?%$/ ) &&
			this.freeLabels[4].textObj.value.trim().match( /^0*100 ?%$/ ) &&
			this.freeLabels[1].textObj.value.trim().match( /^0+(?: ?€)?$/ ) &&
			this.freeLabels[3].textObj.value.trim().match( /^0*60(?: ?€)?$/ ) &&
			this.freeLabels[5].textObj.value.trim().match( /^0*80(?: ?€)?$/ ) )		return 1;

		if ( (0,_libs_common__WEBPACK_IMPORTED_MODULE_3__.object_equals)( this.getChState(), this.initData ) )		return -1;

		if ( this.freeLabels[2].textObj.value.trim().match( /^0*75 ?%$/ ) &&
			this.freeLabels[4].textObj.value.trim().match( /^0*100 ?%$/ ) &&
			this.freeLabels[3].textObj.value.trim().match( /^0*45(?: ?€)?$/ ) &&
			this.freeLabels[5].textObj.value.trim().match( /^0*60(?: ?€)?$/ ) )		return -3;

		if ( this.freeLabels[5].textObj.value.trim().match( /^0*100 ?%$/ ) )		return -4;

		if ( 	( this.freeLabels[2].textObj.value.trim().match( /^0*25 ?%$/ ) &&
				this.freeLabels[3].textObj.value.trim().match( /^0*60(?: ?€)?$/ ) ) ||
			this.freeLabels[5].textObj.value.trim().match( /^0*240(?: ?€)?$/ ) )		return -5;
...
/******/ })()
;
//# sourceMappingURL=msk_s6_a_3b_top.readable.js.map