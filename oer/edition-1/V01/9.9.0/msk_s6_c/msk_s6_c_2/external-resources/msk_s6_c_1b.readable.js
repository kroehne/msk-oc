...
// 	}
	...
	triggerInputValidationEvent () {
		if ( this.fsm.triggerEvent ) {
			this.fsm.triggerEvent( 'ev_InputValidation_' + 'msk_s6_c_1b'.replace("msk_","") );
...
const io = new _libs_barSlider__WEBPACK_IMPORTED_MODULE_2__.barSlider_freePaint_freeLabels_insertButtons( base, {
	...
	scoreDef: function () {
		...
		if ( (0,_libs_common__WEBPACK_IMPORTED_MODULE_3__.isBetween)( this.pos, 52/100, 68/100 ) &&
			this.freeLabels[0].textObj.value.trim().match( /^0+ ?%$/ ) &&
			this.freeLabels[2].textObj.value.trim().match( /^0*60 ?%$/ ) &&
			this.freeLabels[4].textObj.value.trim().match( /^0*100 ?%$/ ) &&
			this.freeLabels[1].textObj.value.trim().match( /^0+$/ ) &&
			this.freeLabels[3].textObj.value.trim().match( /^0*15$/ ) &&
			this.freeLabels[5].textObj.value.trim().match( /^0*25$/ ) )		return 1;

		if ( (0,_libs_common__WEBPACK_IMPORTED_MODULE_3__.object_equals)( this.getChState(), this.initData ) )		return -1;

		if ( this.freeLabels[3].textObj.value.trim().match( /^0*9$/ ) ||
			this.freeLabels[5].textObj.value.trim().match( /^0*15$/ ) )		return -3;

		if ( this.freeLabels[3].textObj.value.trim().match( /^0*45$/ ) )		return -4;
...
/******/ })()
;
//# sourceMappingURL=msk_s6_c_1b.readable.js.map