...
// 	}
	...
	postLog ( event, data={} ) {
		if ( !this.stage || !this.stage.isDemoAni ) {
			...
			if ( true ) {
				console.debug( `Posting event '${event}', message ${JSON.stringify(data)}` );
			}
		}
	}
	...
	triggerInputValidationEvent () {
		if ( this.fsm.triggerEvent ) {
			this.fsm.triggerEvent( 'ev_InputValidation_' + 'msk_s1_a_1a'.replace("msk_","") );
...
/* harmony import */ var _common__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./common */ "./libs/common.js");
...
class ruler {
	...
	constructor ( base, opts = {} ) {
		...
		;(0,_common__WEBPACK_IMPORTED_MODULE_4__.mergeDeep)( Object.assign( this, defaultOpts ), opts );
		this.base = base;
		const stage = base.stage;
		this.stage = stage;

		if ( !this.rulerLayer ) {
			this.rulerLayer = new konva__WEBPACK_IMPORTED_MODULE_0__["default"].Layer();
		...
		this.extraLines.forEach( l => {
			this.rulerLayer.add( new konva__WEBPACK_IMPORTED_MODULE_0__["default"].Line({
				offsetX: l.x,
				x: l.x,
				offsetY: l.y,
				y: l.y,
		...
		// draw ruler
		this.kGroup = new konva__WEBPACK_IMPORTED_MODULE_0__["default"].Group();
		this.rulerLayer.add( this.kGroup );

		const val2x = (val) => this.x+(val+this.offs)*11*lineSpace;
		const kRect = new konva__WEBPACK_IMPORTED_MODULE_0__["default"].Rect({
			x: this.x,
			y: this.y,
			width: this.width,
			height: this.height,
		...
		for ( let v=0; v<=this.maxVal*10; v++ ) {
			...
			this.kGroup.add( new konva__WEBPACK_IMPORTED_MODULE_0__["default"].Line({
				points: [ x, this.y, x, this.y+yl ],
				...this.rulerLineOpts,
			}) );
			if ( !(v%10) ) {
				this.kGroup.add( new konva__WEBPACK_IMPORTED_MODULE_0__["default"].Text({
					x: x-lineSpace*3,
					y: this.y+yl+3,
					width: lineSpace*6,
					text: v/10,
		...
		if ( !this.readonly ) {
			const kTransRect = new konva__WEBPACK_IMPORTED_MODULE_0__["default"].Rect({
				x: this.x,
				y: this.y,
				width: this.width,
				height: this.height,
			...
			kTransRect.on( 'mouseleave', (ev) => {
				if ( (0,_common__WEBPACK_IMPORTED_MODULE_4__.ignoreEvent)( this.stage, ev ) ) {
					return;
				}
			...
			kTransRect.on( 'mousedown touchstart', (ev) => {
				if ( !this.stickyTransform ) {
					this.mouseXOffs = this.kGroup.x() - (0,_common__WEBPACK_IMPORTED_MODULE_4__.getXofEvent)( stage, ev );
					this.mouseYOffs = this.kGroup.y() - (0,_common__WEBPACK_IMPORTED_MODULE_4__.getYofEvent)( stage, ev );
		...
		// rotation point
		const rotateX = val2x( this.rotatePointOpts.xOffs ?? 0 );
		const rotateY = this.y+( this.rotatePointOpts.yOffs ?? this.height/2 );
		this.kRotatePoint = new konva__WEBPACK_IMPORTED_MODULE_0__["default"].Circle({
			x: rotateX,
			y: rotateY,
		...
		if ( !this.readonly ) {
			...
			this.kRotatePoint.on( 'mousedown touchstart', (ev) => {
				if ( !this.stickyRotate ) {
					...
					this.lastMousePos = (0,_common__WEBPACK_IMPORTED_MODULE_4__.getPosOfEvent)( stage, ev );
					this.stickyRotate=1;
				}
		 	});
		}
		...
		if ( !this.readonly ) {
			stage.on( 'mousemove touchmove', (ev) => {
				if ( this.stickyTransform ) {
					const px = Math.min( stage.width()-20, Math.max( 20, (0,_common__WEBPACK_IMPORTED_MODULE_4__.getXofEvent)( stage, ev ) ) );
					this.kGroup.x( this.mouseXOffs + px );
					const py = Math.min( stage.height()-20, Math.max( 20, (0,_common__WEBPACK_IMPORTED_MODULE_4__.getYofEvent)( stage, ev ) ) );
					this.kGroup.y( this.mouseYOffs + py );
				} else if ( this.stickyRotate ) {
					const dx = (0,_common__WEBPACK_IMPORTED_MODULE_4__.getXofEvent)( stage, ev ) - this.mouseXOffs;
					const dy = (0,_common__WEBPACK_IMPORTED_MODULE_4__.getYofEvent)( stage, ev ) - this.mouseYOffs;
...
/* harmony import */ var _libs_ruler__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./libs/ruler */ "./libs/ruler.js");
...
const io = new _libs_ruler__WEBPACK_IMPORTED_MODULE_2__.ruler( base, {
...
/******/ })()
;
//# sourceMappingURL=msk_s1_a_1a.readable.js.map