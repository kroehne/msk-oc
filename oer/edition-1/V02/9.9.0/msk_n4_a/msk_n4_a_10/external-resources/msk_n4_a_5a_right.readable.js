...
// 	}
	...
	triggerInputValidationEvent () {
		if ( this.fsm.triggerEvent ) {
			this.fsm.triggerEvent( 'ev_InputValidation_' + 'msk_n4_a_5a_right'.replace("msk_","") );
...
/////////////////////////////////////////
	scoreDef: function () {
		let erg = -2;
		if ( this.isMultRE( [[3,4],[4,3]], 12 ) ) 	erg = 1;
		if ( !this.div.innerHTML.trim().length ) 	erg = -1;
		if ( this.isMultRE( [[2,3],[3,2]], 6 ) ||
				this.isMultRE( [[3,3]], 9 ) ||
				this.isMultRE( [[3,5],[5,3]], 15 ) ) 	erg = -3;

		if ( this.isMultRE( [[4,5],[5,4]], 20 ) ||
				this.isMultRE( [[2,10],[10,2]], 20 ) ||
				this.isMultRE( [[2,5],[5,2]], 10 ) ||
				this.isMultRE( [[4,8,12]], 384 ) ||
				this.isMultRE( [[4,4,4]], 64 ) ||
				this.isMultRE( [[20,10],[10,20]], 200 ) ) 	erg = -4;

		return {
			[`V_Input_${'5a'}`]: this.extract(),
			[`V_Score_${'5a'}`]: erg,
		}
},
...
/******/ })()
;
//# sourceMappingURL=msk_n4_a_5a_right.readable.js.map