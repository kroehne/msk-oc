...
// 	}
	...
	triggerInputValidationEvent () {
		if ( this.fsm.triggerEvent ) {
			this.fsm.triggerEvent( 'ev_InputValidation_' + 'msk_n4_b_3'.replace("msk_","") );
...
//////////////////////////////////
	scoreDef: function () {
		...
		if ( this.isDivRE( [[15,5]], 3, true ) ) 	erg = 1;

		if ( !this.div.innerHTML.trim().length ) 	erg = -1;

		if ( this.isDivRE( [[3,5],[5,3]], 15, true ) ||
			this.isDivRE( [[15,10]], 5, true ) ||
			this.isDivRE( [[5,10],[10,5]], 15, true ) ) 	erg = -3;

		if ( this.isDivRE( [[5,15]], 3, true ) ) 	erg = -4;

		if ( this.div.innerHTML.trim()=='3' ) 	erg = -5;

		if ( this.isDivRE( [[15,3]], 5, true ) ) 	erg = -6;

		if ( this.isMultRE( [[3,5],[5,3]], 15, true ) ) 	erg = -7;

		return {
			[`V_Input_${'3'}`]: this.extract(),
			[`V_Score_${'3'}`]: erg,
		}
	},
...
/******/ })()
;
//# sourceMappingURL=msk_n4_b_3.readable.js.map