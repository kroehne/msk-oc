  ...
  for (edition in msk_editions){
    for (version in msk_versions){
      ...
      # version <- "V01"
       ...
       files <- list.files(paste0("_content/_aleacodebooks/",edition,"/",version), pattern = "*.xlsx")
       for (f in files){
         # f <- files[1]
         tab <- read.xlsx(xlsxFile=paste0("_content/_aleacodebooks/",edition,"/",version,"/",f), sheet = "Bausteine")
         msk_codebook_title <- rbind(msk_codebook_title,data.frame(Edition=edition, 
          ...
          tab <- read.xlsx(xlsxFile=paste0("_content/_aleacodebooks/",edition,"/",version,"/",f), sheet = "Aufgabenüberschriften")
...
add_ib_project_to_site <- function(file="msk_b1_a_1.zip", 
  ...
  basefolder_url <- "/oer/"
  file <- paste0("_content/_ibprojects/",edition,"/",version,"/",runtime,"/",module,"/",file)
...
for (version in c("V01","V02")){
  for (runtime in c("9.9.0","10.0.0")){
    for (edition in c("edition-1","edition-2")){
      ...
      source_folder <- paste0("_content/_ibprojects/",edition,"/",version,"/",runtime,"/")
...
edition <- "edition-1"
versions <- list.dirs(paste0("_content/_ibprojects/",edition),recursive=F,full.names = F)

for (version in versions){
  
  runtimes <- list.dirs(paste0("_content/_ibprojects/",edition,"/",version),recursive=F,full.names = F)
